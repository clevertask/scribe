...
| ------------------------- | ------------------------------------------------------------------ | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `content`                 | `string`                                                           | `undefined`                | Deprecated for controlled updates. Use this as initial editor content, and prefer `ScribeRef.setContent` for programmatic updates after mount.                                                                                                     |
...
| `enableTableOfContents`   | `boolean`                                                          | `false`                    | Experimental. Enables headless table-of-contents data for heading nodes. Use this with `onTableOfContentsChange` to render your own outline UI outside the editor.                                                                                 |
| `onTableOfContentsChange` | `(items: ScribeTableOfContentsItem[], isCreate?: boolean) => void` | `undefined`                | Experimental. Triggered whenever the heading outline or active/scrolled state changes. Duplicate scroll payloads are ignored. Items include heading text, level, active/scrolled state, position, ID, and the live heading DOM node.               |
...
`ScribeRef` exposes the experimental `scrollToTableOfContentsItem(itemOrId)` helper when `enableTableOfContents` is enabled. The helper scrolls to the current heading element and focuses the editor selection on that heading.
