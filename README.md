...
| `enableTableOfContents`   | `boolean`                                                          | `false`                    | Enables headless table-of-contents data for heading nodes. Use this with `onTableOfContentsChange` to render your own outline UI outside the editor.                                                                                               |
| `onTableOfContentsChange` | `(items: ScribeTableOfContentsItem[], isCreate?: boolean) => void` | `undefined`                | A callback function triggered whenever the heading outline or active/scrolled state changes. Duplicate scroll payloads are ignored. Items include heading text, level, active/scrolled state, position, ID, and the live heading DOM node.         |
...
`ScribeRef` exposes `scrollToTableOfContentsItem(itemOrId)` when `enableTableOfContents` is enabled. Prefer this helper for outline navigation because it resolves the current heading element, focuses the editor without triggering an extra browser scroll, and then scrolls the page to the heading.
